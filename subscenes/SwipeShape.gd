...
func set_shape(shape_array, tile_type = G.TYPE_DOG):
    ...
    for i in range(1, shape_array.size()):
        ...
        if bit == 1:
            ...
            sprite.set_position(Helpers.slot_to_pixels(loc,1,false))	# change false to true to debug position. 1 is xfactor (larger pushes to right and breaks things)
            add_child(sprite)
...
func come_back_to_location(obj, key):
    ...
    if key != ':scale':	# (callback only once per tween)
        return
    var duration = 0.9
    var go_to_loc = HUD.get_node('SavedTileCount').get_global_position()
    var effect = get_node("Tween")
